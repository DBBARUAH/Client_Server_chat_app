...
void set_msg_para(sbcp_msg msg, int H_vrsn, int H_type, int A_type, int A_length){
...
int join(char *pl, int c_sock)
{
  ...
  struct sbcp_msg msg;
  set_msg_para(msg, 3, 2, 2, 2 + 2 + pl_len);//2bytes type, 2bytes length, length of attr payload
...
int send_msg(int c_sock, char *uname)
{
  ...
  struct sbcp_msg msg;
  set_msg_para(msg, 3, 4, 4, 2 + 2 + msg_len);//2bytes type, 2bytes length, length of attr payload
